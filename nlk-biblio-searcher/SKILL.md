...
description: >
  Searches the National Library of Korea OpenAPI (www.nl.go.kr) by title,
  author, or ISBN and extracts bibliographic metadata plus control numbers
  (CNTS-…) for nlk-interlinker. Use when the user asks to search NLK
  holdings or needs control numbers as the front end of the NLK LOD
  pipeline. 키워드: 국립중앙도서관 검색, 서지 검색, 제어번호 확보
version: 1.0.1
status: active
---
...
$PY .skills/nlk-biblio-searcher/scripts/search.py "톨스토이 무저항" --output markdown
$PY .skills/nlk-biblio-searcher/scripts/search.py "본회퍼" --target title --limit 5 --output json
$PY .skills/nlk-biblio-searcher/scripts/search.py --isbn 9788932473901 --output json
```
