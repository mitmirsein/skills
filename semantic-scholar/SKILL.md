...
description: >
  Searches papers and citation networks via the Semantic Scholar Graph API
  (s2_runner.py) and produces normalized Markdown reports with abstracts,
  citation counts, and Open Access links. Pure API — no browser, very fast;
  for browser-rendered Google Scholar use google-scholar-semantic. Use when
  the user asks for S2 search or citation-network recon.
  키워드: 시맨틱 스콜라, S2 검색, 인용 네트워크
author: "MS_Dev"
version: 2.1.0
triggers:
  - "#semanticscholar"
