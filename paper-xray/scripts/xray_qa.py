...
def fix_markdown_emphasis(text: str) -> tuple[str, int]:
    ...
    1. 이탤릭체 사용 금지 규정에 따라 이탤릭(*텍스트*)을 볼드(**텍스트**)로 자동 전환.
       (단, 이미지 경로 내 언더바 충돌을 막기 위해 언더바 이탤릭 변환은 제외하며,
        글머리 기호 리스트 * 등은 보존하기 위해 정교한 정규식을 사용함)
    2. 괄호와 닫는 강조 기호의 결합부()**) 충돌 우회 보정 ()\u200b**\u200b 삽입).
    3. 강조 기호(**) 뒤에 한글/영어가 밀착될 때 렌더링 풀림 방지를 위해 U+200B 삽입.
    """
    count = 0
    
    # 1. 이탤릭체(*텍스트*) -> 볼드(**텍스트**) 변환 (최대 50자, 공백 시작 불가)
    pattern_italic_star = r'(?<!\*)\*(?!\s)([^*]{1,50}?\S)\*(?!\*)'
    text, c1 = re.subn(pattern_italic_star, r'**\1**', text)
    count += c1

    # 2. 괄호 뒤 볼드 닫기 충돌 보정: )**로 -> )\u200b**\u200b로
    pattern_paren = r'(\))\*\*(?!\u200b)([가-힣a-zA-Z])'
    text, c2 = re.subn(pattern_paren, r'\1' + '\u200b' + r'**' + '\u200b' + r'\2', text)
    count += c2

    # 3. 괄호 뒤 볼드 닫기 기호 바로 앞에도 U+200B가 없는 경우 보정: )** -> )\u200b**
    pattern_paren_direct = r'(\))(?!\u200b)\*\*'
    text, c3 = re.subn(pattern_paren_direct, r'\1' + '\u200b' + r'**', text)
    count += c3

    # 4. 일반적인 볼드 기호 뒤 결합 보정: **단어 -> **\u200b단어 / **조사 -> **\u200b조사
