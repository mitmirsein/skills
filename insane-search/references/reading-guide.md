# 관련 문서 선택 가이드 — 언제 무엇을 읽을지 (정본)

문제가 생겼을 때 어떤 `references/*.md`를 열어야 할지 결정하는 기준. Claude는 필요할 때만 해당 파일을 `Read`하고, 선제적으로 전부 읽지 않는다.
...
| [`theology-scholar-sweep.md`](theology-scholar-sweep.md) | 신학 학술 논문을 넓게 찾고 교차검증할 때 | Google Scholar Semantic, S2 API, IxTheo, CrossRef Journal, KCI를 순차/병렬로 탐색하는 로컬 학술 sweep |
| [`kci.md`](kci.md) | KCI/국내 논문 검색·상세 조회 시 | `.skills/kci-api-searcher` CLI, dev `.env`의 KCI API 키, 검색 흐름 |
| [`paywall.md`](paywall.md) | 페이월/구독벽 감지 패턴을 별도로 확인할 때 | 로컬 확장 문서. 사용 전 법적·약관·저작권 리스크와 상위 정책을 우선 확인 |
